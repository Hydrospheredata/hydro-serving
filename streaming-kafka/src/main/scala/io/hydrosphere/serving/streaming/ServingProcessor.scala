...
object SidecarServingProcessor {
  ...
  def apply(sidecarConfig: SidecarConfig, serviceName: String)
    (implicit sys: ActorSystem, mat: ActorMaterializer): SidecarServingProcessor =
    SidecarServingProcessor(new HttpRuntimeMeshConnector(sidecarConfig), serviceName)

  def apply(connector: RuntimeMeshConnector, serviceName: String): SidecarServingProcessor = {
    val ec = ExecutionContext.fromExecutor(Executors.newFixedThreadPool(10))
    new SidecarServingProcessor(connector, serviceName, ec)
  }
