...
_Runtimes_ are services with the predefined infrasturcture. They must implement a set if specific methods that are used by manager. They run in the docker environment and are responsible for running user-defined models. When you create a new application and assign a model to it, you also have to provide a specific runtime, which will be used to run your model. 
...
Let's quickly review, what we have here. `RuntimeManager` simply manages our service, i.e. starts it, stops it, holds all necessary data. `RuntimeService` is our service that actually implements `Predict(PredictRequest)`.

The model will be stored inside the `/model` directory in the docker conrtainersubli. The structure of `/model` is following: 
...
CMD ["python", "main.py"]
```
`APP_PORT` is an environment variable that will be created by manager. When the PredictRequest is generated it goes directly to that port. Although we specify the variable here, we actually don't have to as it's going to be generated automatically by manager, but we keep it as a backup.
...
The `username` should be the one you have registered in Docker Hub. Building and pushing image might take a while depending on your internet connection.

### Fetching Runtime

To add your newly created runtime just execute the following lines.
...
Here, `name` is your image published in the Docker Hub. `version` is the tag of that image. The last parameter (addess) is where your ML Lambda instance is running.

### Result

That's it. You've just created a simple runtime, that you can use in your own projects. This is an almost equal version of our [python runtime implementation][github-python-runtime]. You can always look up details there. 
