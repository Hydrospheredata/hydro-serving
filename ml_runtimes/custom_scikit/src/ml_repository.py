...
class MLRepository:
    ...
    def request_json(url):
        jss = urllib.request.urlopen(url).read().decode('utf8')
        return json.loads(jss)

    def get_metadata(self, model_name):
        return MLRepository.request_json("{0}/metadata/{1}".format(self.host, model_name))

    def get_files(self, model_name):
        return MLRepository.request_json("{0}/files/{1}".format(self.host, model_name))
