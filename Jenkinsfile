...
node("JenkinsOnDemand") {
    ...
    stage('Test') {
        try {
            def curVersion = currentVersion()
            sh "${env.WORKSPACE}/sbt/sbt -DappVersion=${curVersion} -Dsbt.override.build.repos=true -Dsbt.repository.config=${env.WORKSPACE}/project/repositories test it:testOnly"
        } finally {
