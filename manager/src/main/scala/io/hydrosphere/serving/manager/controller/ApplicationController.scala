...
class ApplicationController(
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        value         = "Application",
        required      = true,
        dataTypeClass = classOf[ApplicationCreateOrUpdateRequest],
        paramType     = "body"
      )
    )
  )
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        name          = "body",
        value         = "ApplicationCreateOrUpdateRequest",
        required      = true,
        dataTypeClass = classOf[ApplicationCreateOrUpdateRequest],
        paramType     = "body"
      )
  ...
  @ApiOperation(
    value      = "Generate payload for application",
    notes      = "Generate payload for application",
    nickname   = "Generate payload for application",
    httpMethod = "GET"
  )
  @ApiImplicitParams(
    Array(
  ...
  def generateInputsForApp =
    path("api" / "v1" / "applications" / "generateInputs" / LongNumber / Segment) {
      (appId, signatureName) =>
        get {
          complete(
  ...
  @ApiOperation(
    value      = "Serve Application by id",
    notes      = "Serve Application by id",
    nickname   = "Serve Application by id",
    httpMethod = "POST"
  )
  @ApiImplicitParams(
    Array(
  ...
  def serveById = path("api" / "v1" / "applications" / "serve" / LongNumber / Segment) {
    (serviceId, signatureName) =>
      post {
