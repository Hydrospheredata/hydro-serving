...
object TracingHeaders {
  ...
  val allHeaders =
    Seq(xOtSpanContext, xRequestId, xB3TraceId, xB3SpanId, xB3ParentSpanId, xB3Sampled, xB3Flags)

  def isTracingHeaderName(header: String): Boolean = header.toLowerCase match {
    case `xOtSpanContext` | `xRequestId` | `xB3TraceId` | `xB3SpanId` | `xB3ParentSpanId` |
        `xB3Sampled` | `xB3Flags` =>
      true
    case _ => false
  }
