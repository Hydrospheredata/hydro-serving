...
class ServiceRepositoryImpl(
  ...
  override def getLastServiceByModelNameAndVersion(
    modelName: String,
    modelVersion: Long
    ...
    db.run(
        Tables.Service
          ...
          .filter {
            case ((_, mv), _) =>
              mv.map(_.modelName) === modelName && mv.map(_.modelVersion) === modelVersion
          }
          .result
          .headOption
      )
...
object ServiceRepositoryImpl {
  ...
  def mapFromDb(
    model: Option[
      (
        (
          (
            (Tables.Service#TableElementType, Option[Tables.Runtime#TableElementType]),
            Option[Tables.Environment#TableElementType]
          ),
          Option[Tables.ModelVersion#TableElementType]
        ),
        Option[Tables.Model#TableElementType]
      )
    ]
  ): Option[Service] =
    model.map(mapFromDb)
      ...
      case ((((service, runtime), environment), modelVersion), model) =>
        mapFromDb(
          ...
          RuntimeRepositoryImpl.mapFromDb(
            runtime.getOrElse(
              throw new RuntimeException(s"Can't find Runtime for service ${service.serviceId}")
            )
          ),
          EnvironmentRepositoryImpl.mapFromDb(environment),
          modelVersion.map(
            s => ModelVersionRepositoryImpl.mapFromDb(s, ModelRepositoryImpl.mapFromDb(model))
          )
        )
    }
  }
  ...
  def mapFromDb(
    ...
    Service(
      ...
      configParams = model.configParams
        .map(s => {
          val arr = s.split('=')
