...
class ListenerDSActor
  extends AbstractDSActor[Listener](typeUrl = "type.googleapis.com/envoy.api.v2.Listener") {
  ...
  private def createHttpConnectionManagerFilter(
    name: String,
  ...
  private def createListener(port: Int, operationName: OperationName): Listener = {
    Listener(
      ...
      filterChains = Seq(
        FilterChain(
          filterChainMatch = Some(
            FilterChainMatch(
              )
          ),
          filters =
            Seq(createHttpConnectionManagerFilter(operationName.name.toLowerCase, operationName))
        )
      )
    )
  }
