...
class LocalCloudDriverService(
  ...
  private def startModel(service: Service): String = {
    ...
    val container = dockerClient.createContainer(
      ContainerConfig
        ...
        .labels(javaLabels)
        .build(),
      generateModelContainerName(service)
    )
    container.id()
  }
  ...
  override def deployService(service: Service): Future[CloudService] =
    Future.apply({
      ...
      val javaLabels = mapAsJavaMap(
        getRuntimeLabels(service) ++ Map(
      ...
      val c = dockerClient.createContainer(
        ContainerConfig
          .builder()
          ...
          .build(),
        s"s${service.id}app${service.serviceName}"
      )
  ...
  protected def mapToCloudService(serviceId: Long, seq: Seq[Container]): CloudService = {
    ...
    val containerApp = map.getOrElse(
      DEPLOYMENT_TYPE_APP,
      throw new RuntimeException(s"Can't find APP for service $serviceId in $seq")
    )
