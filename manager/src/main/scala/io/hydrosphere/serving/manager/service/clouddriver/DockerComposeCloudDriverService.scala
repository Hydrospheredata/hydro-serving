...
class DockerComposeCloudDriverService(
...
) extends LocalCloudDriverService(
    dockerClient,
    managerConfiguration,
    internalManagerEventsPublisher
  ) {

  private val driverConfiguration =
    managerConfiguration.cloudDriver.asInstanceOf[DockerCloudDriverConfiguration]

  override protected def postProcessAllServiceList(
    services: Seq[CloudService]
  ): Seq[CloudService] = {
    val manager = services
      .find(_.id == CloudDriverService.MANAGER_ID)
      .getOrElse(throw new RuntimeException(s"Can't find manager in $services"))

    services.filter(_.id != CloudDriverService.MANAGER_ID) :+ createManagementHttp(manager) :+ createManagement(
      manager
    )
  }

  override protected def createMainApplicationHostConfigBuilder(): HostConfig.Builder = {
    val builder = HostConfig
      .builder()
  ...
  private def createManagementHttp(management: CloudService): CloudService =
    management.copy(
      ...
      instances = management.instances.map(
        s =>
          s.copy(
      ...
      instances = management.instances.map(
        s =>
          s.copy(
            ...
            mainApplication =
              s.mainApplication.copy(port = managerConfiguration.application.grpcPort)
        )
      )
    )

  override protected def mapMainApplicationInstance(
