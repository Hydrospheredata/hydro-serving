...
import io.hydrosphere.serving.manager.model.api.json.FloatToJson
...
class InfoFieldValidator(val field: ModelField, val dataType: DataType) {
  ...
  def process(data: Seq[JsValue]): Either[ValidationError, TensorProto] = {
    ...
    val convertedData = TypedTensorFactory(dataType) match {
      case FloatTensor => FloatToJson.toJson
